...
async function create(req, res, next) { try { const payload = { ...req.body, ownerUid: req.user.uid }; res.status(201).json(await svc.create(payload)); } catch(e){next(e);} }
async function list(req, res, next) { try { res.json(await svc.list({ ownerUid: req.user.uid })); } catch(e){next(e);} }
